...
{
	[MessageType("DD", FixVersion.FIX50SP2)]
	public sealed partial class TradeMatchReportAck : IFixMessage
	{
		...
		[TagDetails(Tag = 1896, Type = TagType.Int, Offset = 3, Required = true)]
		public int? TradeMatchAckStatus {get; set;}
		
		[TagDetails(Tag = 1897, Type = TagType.Int, Offset = 4, Required = false)]
		public int? TradeMatchRejectReason {get; set;}
		
		[TagDetails(Tag = 1328, Type = TagType.String, Offset = 5, Required = false)]
		public string? RejectText {get; set;}
		
		[TagDetails(Tag = 1664, Type = TagType.Length, Offset = 6, Required = false, LinksToTag = 1665)]
		public int? EncodedRejectTextLen {get; set;}
		
		[TagDetails(Tag = 1665, Type = TagType.RawData, Offset = 7, Required = false, LinksToTag = 1664)]
		...
		bool IFixValidator.IsValid(in FixValidatorConfig config)
		{
			return
				...
				&& TrdMatchID is not null
				&& TradeMatchAckStatus is not null
		...
		void IFixEncoder.Encode(IFixWriter writer)
		{
			...
			if (TradeMatchAckStatus is not null) writer.WriteWholeNumber(1896, TradeMatchAckStatus.Value);
			if (TradeMatchRejectReason is not null) writer.WriteWholeNumber(1897, TradeMatchRejectReason.Value);
		...
		void IFixParser.Parse(IMessageView? view)
		{
			...
			TradeMatchAckStatus = view.GetInt32(1896);
			TradeMatchRejectReason = view.GetInt32(1897);
		...
		bool IFixLookup.TryGetByTag(string name, out object? value)
		{
			...
			switch (name)
			{
				...
				case "TrdMatchID":
					value = TrdMatchID;
					break;
				case "TradeMatchAckStatus":
					value = TradeMatchAckStatus;
					break;
				case "TradeMatchRejectReason":
					value = TradeMatchRejectReason;
					break;
				case "RejectText":
					value = RejectText;
					break;
		...
		void IFixReset.Reset()
		{
			...
			TrdMatchID = null;
			TradeMatchAckStatus = null;
			TradeMatchRejectReason = null;
			RejectText = null;
