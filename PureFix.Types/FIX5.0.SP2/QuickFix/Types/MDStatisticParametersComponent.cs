...
{
	public sealed partial class MDStatisticParametersComponent : IFixComponent
	{
		...
		[TagDetails(Tag = 2481, Type = TagType.Length, Offset = 6, Required = false, LinksToTag = 2482)]
		public int? EncodedMDStatisticDescLen {get; set;}
		
		[TagDetails(Tag = 2482, Type = TagType.RawData, Offset = 7, Required = false, LinksToTag = 2481)]
		...
		{
			return
				MDStatisticType is not null
				&& MDStatisticScope is not null
				&& MDStatisticIntervalType is not null;
		}
		...
		void IFixEncoder.Encode(IFixWriter writer)
		{
			...
			if (EncodedMDStatisticDesc is not null)
			{
				writer.WriteWholeNumber(2481, EncodedMDStatisticDesc.Length);
				writer.WriteBuffer(2482, EncodedMDStatisticDesc);
			}
