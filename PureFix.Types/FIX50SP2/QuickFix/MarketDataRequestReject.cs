...
namespace PureFix.Types.FIX50SP2.QuickFix
{
	...
	public sealed partial class MarketDataRequestReject : IFixMessage
	{
		...
		[TagDetails(Tag = 281, Type = TagType.String, Offset = 3, Required = false)]
		public string? MDReqRejReason {get; set;}
		
		[Component(Offset = 4, Required = false)]
		public MDRjctGrpComponent? MDRjctGrp {get; set;}
		...
		void IFixParser.Parse(IMessageView? view)
		{
			...
			if (view.GetView("MDRjctGrp") is IMessageView viewMDRjctGrp)
			{
				MDRjctGrp = new();
				((IFixParser)MDRjctGrp).Parse(viewMDRjctGrp);
			}
