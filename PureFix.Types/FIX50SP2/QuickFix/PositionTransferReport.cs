...
namespace PureFix.Types.FIX50SP2.QuickFix
{
	...
	public sealed partial class PositionTransferReport : IFixMessage
	{
		...
		[TagDetails(Tag = 1664, Type = TagType.Length, Offset = 23, Required = false, LinksToTag = 1665)]
		public int? EncodedRejectTextLen {get; set;}
		
		[TagDetails(Tag = 1665, Type = TagType.RawData, Offset = 24, Required = false, LinksToTag = 1664)]
		...
		bool IFixValidator.IsValid(in FixValidatorConfig config)
		{
			return
				...
				&& TransferReportID is not null
				&& TransferID is not null
				&& TransferTransType is not null
				&& TransferReportType is not null
				&& TransferStatus is not null
				&& Parties is not null && ((IFixValidator)Parties).IsValid(in config)
				&& TargetParties is not null && ((IFixValidator)TargetParties).IsValid(in config)
