...
{
	public sealed partial class IOINoEvents : IFixGroup
	{
		...
		[TagDetails(Tag = 1578, Type = TagType.Length, Offset = 8, Required = false, LinksToTag = 1579)]
		public int? EncodedEventTextLen {get; set;}
		
		[TagDetails(Tag = 1579, Type = TagType.RawData, Offset = 9, Required = false, LinksToTag = 1578)]
		...
		void IFixEncoder.Encode(IFixWriter writer)
		{
			...
			if (EncodedEventText is not null)
			{
				writer.WriteWholeNumber(1578, EncodedEventText.Length);
				writer.WriteBuffer(1579, EncodedEventText);
			}
		}
