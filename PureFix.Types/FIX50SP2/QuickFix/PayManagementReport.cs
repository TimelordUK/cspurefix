...
namespace PureFix.Types.FIX50SP2.QuickFix
{
	...
	public sealed partial class PayManagementReport : IFixMessage
	{
		...
		[TagDetails(Tag = 2802, Type = TagType.Length, Offset = 6, Required = false, LinksToTag = 2801)]
		public int? EncodedReplaceTextLen {get; set;}
		
		[TagDetails(Tag = 2801, Type = TagType.RawData, Offset = 7, Required = false, LinksToTag = 2802)]
		...
		[TagDetails(Tag = 1664, Type = TagType.Length, Offset = 11, Required = false, LinksToTag = 1665)]
		public int? EncodedRejectTextLen {get; set;}
		
		[TagDetails(Tag = 1665, Type = TagType.RawData, Offset = 12, Required = false, LinksToTag = 1664)]
		...
		public PostTradePaymentComponent? PostTradePayment {get; set;}
		...
		bool IFixValidator.IsValid(in FixValidatorConfig config)
		{
			return
				...
				&& PayReportTransType is not null
				&& TransactTime is not null
				&& PostTradePayment is not null && ((IFixValidator)PostTradePayment).IsValid(in config)
		...
		void IFixEncoder.Encode(IFixWriter writer)
		{
			...
			if (EncodedReplaceText is not null)
			{
				writer.WriteWholeNumber(2802, EncodedReplaceText.Length);
				writer.WriteBuffer(2801, EncodedReplaceText);
			}
