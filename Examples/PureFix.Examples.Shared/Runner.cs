...
public static class Runner
{
    ...
    /// </summary>
    public static async Task Run(string acceptorConfigPath, string initiatorConfigPath, string dictRootPath, IFixClock clock, Func<IFixClock, IFixConfig, BaseAppDI> makeHost, string? storeDirectory = null)
    {
        var acceptor = Start(acceptorConfigPath, dictRootPath, clock, makeHost, storeDirectory);
        await Task.Delay(500); // Let acceptor start listening
        var initiator = Start(initiatorConfigPath, dictRootPath, clock, makeHost, storeDirectory);
    ...
    public static async Task RunSingle(string configPath, string dictRootPath, IFixClock clock, Func<IFixClock, IFixConfig, BaseAppDI> makeHost, string? storeDirectory = null)
    {
        var task = Start(configPath, dictRootPath, clock, makeHost, storeDirectory);
        await task;
    }

    private static Task Start(string configPath, string dictRootPath, IFixClock clock, Func<IFixClock, IFixConfig, BaseAppDI> makeHost, string? storeDirectory = null)
    {
