...
{
	public sealed partial class IOINoUnderlyingEvents : IFixGroup
	{
		...
		[TagDetails(Tag = 2072, Type = TagType.Length, Offset = 8, Required = false, LinksToTag = 2073)]
		public int? EncodedUnderlyingEventTextLen {get; set;}
		
		[TagDetails(Tag = 2073, Type = TagType.RawData, Offset = 9, Required = false, LinksToTag = 2072)]
		...
		void IFixEncoder.Encode(IFixWriter writer)
		{
			...
			if (EncodedUnderlyingEventText is not null)
			{
				writer.WriteWholeNumber(2072, EncodedUnderlyingEventText.Length);
				writer.WriteBuffer(2073, EncodedUnderlyingEventText);
			}
		}
