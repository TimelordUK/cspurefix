...
{
	[MessageType("CollateralReport", FixVersion.FIX50SP2)]
	public sealed partial class CollateralReport : IFixMessage
	{
		...
		public CollateralReportParties[]? Parties {get; set;}
		...
		[TagDetails(Tag = 53, Type = TagType.Float, Offset = 19, Required = false)]
		...
		[Group(NoOfTag = 1020, Offset = 27, Required = false)]
		public CollateralReportTrdRegTimestamps[]? TrdRegTimestamps {get; set;}
		...
		[Group(NoOfTag = 1019, Offset = 36, Required = false)]
		public CollateralReportStipulations[]? Stipulations {get; set;}
		...
		[TagDetails(Tag = 354, Type = TagType.Length, Offset = 44, Required = false, LinksToTag = 355)]
		public int? EncodedTextLen {get; set;}
		
		[TagDetails(Tag = 355, Type = TagType.RawData, Offset = 45, Required = false, LinksToTag = 354)]
		...
		bool IFixValidator.IsValid(in FixValidatorConfig config)
		{
			return
				...
				&& CollRptID is not null
				&& CollStatus is not null
		...
		void IFixParser.Parse(IMessageView? view)
		{
			...
			if (view.GetView("Parties") is IMessageView viewParties)
			{
				...
				Parties = new CollateralReportParties[count];
			...
			if (view.GetView("TrdRegTimestamps") is IMessageView viewTrdRegTimestamps)
			{
				...
				TrdRegTimestamps = new CollateralReportTrdRegTimestamps[count];
				for (int i = 0; i < count; i++)
				{
			...
			if (view.GetView("Stipulations") is IMessageView viewStipulations)
			{
				...
				Stipulations = new CollateralReportStipulations[count];
				for (int i = 0; i < count; i++)
				{
					Stipulations[i] = new();
