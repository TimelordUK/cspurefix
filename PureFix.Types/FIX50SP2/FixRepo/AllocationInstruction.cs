...
{
	[MessageType("AllocationInstruction", FixVersion.FIX50SP2)]
	public sealed partial class AllocationInstruction : IFixMessage
	{
		...
		public AllocationInstructionParties[]? Parties {get; set;}
		...
		public AllocationInstructionStipulations[]? Stipulations {get; set;}
		
		[Component(Offset = 58, Required = false)]
		public YieldDataComponent? YieldData {get; set;}
		
		[Group(NoOfTag = 1014, Offset = 59, Required = false)]
		public AllocationInstructionPositionAmountData[]? PositionAmountData {get; set;}
		...
		[Group(NoOfTag = 1062, Offset = 72, Required = false)]
		public AllocationInstructionRateSource[]? RateSource {get; set;}
		...
		void IFixParser.Parse(IMessageView? view)
		{
			...
			if (view.GetView("Parties") is IMessageView viewParties)
			{
				...
				Parties = new AllocationInstructionParties[count];
			...
			if (view.GetView("Stipulations") is IMessageView viewStipulations)
			{
				...
				Stipulations = new AllocationInstructionStipulations[count];
				for (int i = 0; i < count; i++)
				{
					Stipulations[i] = new();
			...
			if (view.GetView("PositionAmountData") is IMessageView viewPositionAmountData)
			{
				...
				PositionAmountData = new AllocationInstructionPositionAmountData[count];
				for (int i = 0; i < count; i++)
				{
			...
			if (view.GetView("RateSource") is IMessageView viewRateSource)
			{
				...
				RateSource = new AllocationInstructionRateSource[count];
				for (int i = 0; i < count; i++)
				{
					RateSource[i] = new();
