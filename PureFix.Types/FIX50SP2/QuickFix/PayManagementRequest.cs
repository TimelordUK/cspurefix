...
namespace PureFix.Types.FIX50SP2.QuickFix
{
	...
	public sealed partial class PayManagementRequest : IFixMessage
	{
		...
		[TagDetails(Tag = 2809, Type = TagType.Length, Offset = 5, Required = false, LinksToTag = 2808)]
		public int? EncodedCancelTextLen {get; set;}
		
		[TagDetails(Tag = 2808, Type = TagType.RawData, Offset = 6, Required = false, LinksToTag = 2809)]
		...
		bool IFixValidator.IsValid(in FixValidatorConfig config)
		{
			return
				...
				&& PayRequestID is not null
				&& PayRequestTransType is not null
		...
		void IFixEncoder.Encode(IFixWriter writer)
		{
			...
			if (EncodedCancelText is not null)
			{
				writer.WriteWholeNumber(2809, EncodedCancelText.Length);
				writer.WriteBuffer(2808, EncodedCancelText);
			}
