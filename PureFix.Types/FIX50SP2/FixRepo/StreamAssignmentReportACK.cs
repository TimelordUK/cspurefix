...
{
	[MessageType("StreamAssignmentReportACK", FixVersion.FIX50SP2)]
	public sealed partial class StreamAssignmentReportACK : IFixMessage
	{
		[Component(Offset = 0, Required = true)]
		public StandardHeaderComponent? StandardHeader {get; set;}
		
		[TagDetails(Tag = 1503, Type = TagType.Int, Offset = 1, Required = true)]
		public int? StreamAsgnAckType {get; set;}
		
		[TagDetails(Tag = 1501, Type = TagType.String, Offset = 2, Required = true)]
		public string? StreamAsgnRptID {get; set;}
		
		[TagDetails(Tag = 1502, Type = TagType.Int, Offset = 3, Required = false)]
		...
		bool IFixValidator.IsValid(in FixValidatorConfig config)
		{
			return
				...
				&& StreamAsgnAckType is not null
				&& StreamAsgnRptID is not null
		...
		void IFixEncoder.Encode(IFixWriter writer)
		{
			...
			if (StreamAsgnAckType is not null) writer.WriteWholeNumber(1503, StreamAsgnAckType.Value);
		...
		void IFixParser.Parse(IMessageView? view)
		{
			...
			StreamAsgnAckType = view.GetInt32(1503);
		...
		bool IFixLookup.TryGetByTag(string name, out object? value)
		{
			...
			switch (name)
			{
				case "StandardHeader":
					...
					break;
				case "StreamAsgnAckType":
					value = StreamAsgnAckType;
					break;
