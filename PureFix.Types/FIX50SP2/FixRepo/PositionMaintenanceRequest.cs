...
{
	[MessageType("PositionMaintenanceRequest", FixVersion.FIX50SP2)]
	public sealed partial class PositionMaintenanceRequest : IFixMessage
	{
		...
		[Group(NoOfTag = 1012, Offset = 9, Required = true)]
		public PositionMaintenanceRequestParties[]? Parties {get; set;}
		...
		[Group(NoOfTag = 1015, Offset = 18, Required = true)]
		public PositionMaintenanceRequestPositionQty[]? PositionQty {get; set;}
		
		[Group(NoOfTag = 1014, Offset = 19, Required = false)]
		public PositionMaintenanceRequestPositionAmountData[]? PositionAmountData {get; set;}
		...
		[TagDetails(Tag = 120, Type = TagType.String, Offset = 27, Required = false)]
		...
		void IFixParser.Parse(IMessageView? view)
		{
			...
			if (view.GetView("Parties") is IMessageView viewParties)
			{
				...
				Parties = new PositionMaintenanceRequestParties[count];
			...
			if (view.GetView("PositionQty") is IMessageView viewPositionQty)
			{
				...
				PositionQty = new PositionMaintenanceRequestPositionQty[count];
				for (int i = 0; i < count; i++)
				{
					PositionQty[i] = new();
			...
			if (view.GetView("PositionAmountData") is IMessageView viewPositionAmountData)
			{
				...
				PositionAmountData = new PositionMaintenanceRequestPositionAmountData[count];
				for (int i = 0; i < count; i++)
				{
