...
{
	public sealed partial class IOINoLegEvents : IFixGroup
	{
		...
		[TagDetails(Tag = 2074, Type = TagType.Length, Offset = 8, Required = false, LinksToTag = 2075)]
		public int? EncodedLegEventTextLen {get; set;}
		
		[TagDetails(Tag = 2075, Type = TagType.RawData, Offset = 9, Required = false, LinksToTag = 2074)]
		...
		void IFixEncoder.Encode(IFixWriter writer)
		{
			...
			if (EncodedLegEventText is not null)
			{
				writer.WriteWholeNumber(2074, EncodedLegEventText.Length);
				writer.WriteBuffer(2075, EncodedLegEventText);
			}
		}
